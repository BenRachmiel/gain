...
use super::{DownloadSource, SearchAlbum, SourceType, scan_artists};
...
impl TidalSource {
    ...
    async fn resolve_track_url(state: &AppState, track_id: u64) -> Option<String> {
        for mirror in &state.config.source_mirrors {
                ...
                .await
            {
                Ok(resp) => {
                    ...
                    let manifest_b64 =
                        match data.pointer("/data/manifest").and_then(|v| v.as_str()) {
                            Some(m) => m,
                            None => continue,
                        };
...
impl DownloadSource for TidalSource {
    async fn search(
        &self,
        query: &str,
        _page: u32,
        ...
        let data = Self::source_get(state, "/search/", &[("al", query)]).await?;
        ...
        let albums: Vec<SearchAlbum> = items
            .into_iter()
            .filter_map(|item| {
    ...
    async fn resolve_album(
        ...
        let data = Self::source_get(state, "/album/", &[("id", album_id)]).await?;
        let album_data = data.get("data").ok_or("Missing data field")?.clone();
        ...
        tokio::spawn(async move {
            ...
            let matched =
                util::match_artist(&artist, &existing_artists).map(|s| s.to_string());
            ...
            let total_discs = album_data["numberOfVolumes"].as_u64().unwrap_or(1) as u32;
            ...
            for item_wrapper in &items {
                ...
                let disc = item["volumeNumber"].as_u64().unwrap_or(1) as u32;
                let duration_s = item["duration"].as_u64().unwrap_or(0);
                let mins = duration_s / 60;
                let secs = duration_s % 60;

                let title = item["title"].as_str().unwrap_or("");
                match Self::resolve_track_url(&state, track_id).await {
                    Some(url) => {
                        let track = json!({
                            "index": idx,
                            "disc": disc,
